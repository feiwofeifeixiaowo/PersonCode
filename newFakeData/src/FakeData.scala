  ...
  object FakeData {
    ...
    def main(args: Array[String]) {
      ...
      val sourceRDD = sc.textFile("file:///Users/X/Git_Repo/PersonCode/data/" + number + ".log")
      var idArray = ArrayBuffer[String]()
      val bcArray: Broadcast[ArrayBuffer[String]] = sc.broadcast(idArray)
      val dataRDD   = sourceRDD.flatMap( line => {
        ...
        for (i <- 1 until (items.length -1)) {
          ...
          bcArray.value.append(id)
          retArray.append(Tuple2(id, value))
        }
        retArray
      }).reduceByKey(_+ "@" +_)//.sortByKey()
      // out put to file
//       dataRDD.repartition(1).saveAsTextFile("file:///Users/X/Desktop/midResult")

      // select new id in random way
      val r     = scala.util.Random
      val idVec = dataRDD.collect().toVector
      val idList = idArray.toSet.toList
      println("ID库的长度为： idVec-->>" + idVec.size)
      println("NEW ID库的长度为： bcArray-->>" + idArray.length)
      println("NEW ID库的长度为： bcArray-->>" + idList.length)
